...
//		key_array[i] = reverse(current);
	...
	char out_str[data_length*20];
    int str_index = 0;

    __android_log_print(ANDROID_LOG_DEBUG, "ndktest_jni", "\nFabric out before encryption:\n");
    for(i=0; i<data_length; i++){
        out_str[str_index] = 'x';
        str_index++;
        for(j=0; j<16; j++){
            ...
            char str_buf[20];
            sprintf(str_buf, "%02x", fabric);
            int k;
            for(k = 0; k<strlen(str_buf); k++){
...
//	printf("\nOpenssl:\n");
	...
	ticks = clock();
	for(i=0; i<data_length; i++){
//		__android_log_print(ANDROID_LOG_DEBUG, "ndktest_jni", "0x");
        out_str[str_index] = 'x';
        str_index++;
		for(j=0; j<16; j++){
